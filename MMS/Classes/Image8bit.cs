    ...
    public class Image8bit
    {
        ...
        private int GetPixelInfoSize(PixelFormat format)
        {
            switch (format)
            {
                ...
                default:
                    {
                        MessageBox.Show("You can only convert 24 bit images.");
                        throw new ApplicationException("Only 24bit colors supported now");
                        //MetroMessageBox.Show(this, "Error", "You can only convert 24 bit bitmaps.", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
            }
